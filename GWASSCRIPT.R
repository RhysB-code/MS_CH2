...
if (!requireNamespace("GWASpoly", quietly = TRUE)) {
  ...
  install_github("jendelman/GWASpoly")  # Install GWASpoly from GitHub
}

library(dplyr)   # Load dplyr for data manipulation
library(GWASpoly) # Load GWASpoly for GWAS analysis
library(openxlsx) # Load openxlsx for reading/writing Excel files

# Set working directory
setwd("C:/Users/RhysB/Downloads")

# ================================
# 2. Genotype File Preparation
# ================================
# Read the SNP data
snp_raw <- read.csv("Filt5_80k_noPBBwithcodes.csv", skip = 1, fileEncoding = "latin1", check.names = FALSE)
...
names(snp_raw)[1:5] <- c("Marker", "MarkerGDR", "Chrom", "Position", "Ref_Alt")

# Select relevant columns and convert genotype values to numeric
geno_full <- snp_raw %>%
  select(Marker, Chrom, Position, everything()) %>%
  select(-MarkerGDR, -Ref_Alt)

# Convert genotype values to numeric
geno_only <- geno_full[, 4:ncol(geno_full)]
geno_only[] <- lapply(geno_only, function(x) as.numeric(as.character(x)))

# Remove markers with more than 25% missing data
geno_full_clean <- bind_cols(geno_full[, 1:3], geno_only) %>%
  filter(rowSums(is.na(.)) < 0.25 * ncol(.))

# Save cleaned genotype data to a file
write.table(geno_full_clean, "Genotype_full.txt", sep = "\t", quote = FALSE, row.names = FALSE)

# ================================
# 3. Phenotype File Preparation
# ================================
# Read the phenotype data and select relevant columns
pheno <- read.csv("Phenotype.csv") %>%
  select(Genotype, Budbreak_Jan1, Budbreak_Jan15, Floral_Jan1, Floral_Jan15) %>%
  na.omit()  # Remove rows with NA values

# Save cleaned phenotype data to a file
write.csv(pheno, "Phenotype_clean.csv", row.names = FALSE)

# ================================
# 4. Run GWASpoly for One Trait
# ================================
# Read genotype and phenotype data into GWASpoly format
data <- read.GWASpoly(
  ploidy = 4,
  pheno.file = "Phenotype_clean.csv",
  geno.file = "Genotype_full.txt",
  format = "numeric"
)

# Check if data was read correctly
if (!inherits(data, "GWASpoly.data")) {
  stop("Error: Data was not read correctly. Please check your input files.")
}

# Set the kinship matrix
data <- set.K(data)

# Define the trait to analyze
trait <- "Budbreak_Jan15"

# Set the model for GWAS
data <- set.model(data, trait = trait, models = "general")

# Run GWAS analysis
data <- run.GWASpoly(data)

# ================================
# 5. Plot Manhattan and QQ Plots
# ================================
# Check if data contains results before plotting
if (!is.null(data$results)) {
  # Show plots
  plot(data, trait = trait, model = "general", type = "manhattan")
  plot(data, trait = trait, model = "general", type = "qq")
  
  # Save Manhattan plot to file
  png("Manhattan_Budbreak_Jan15.png", width = 1000, height = 600)
  plot(data, trait = trait, model = "general", type = "manhattan")
  dev.off()
  
  # Save QQ plot to file
  png("QQ_Budbreak_Jan15.png", width = 800, height = 600)
  plot(data, trait = trait, model = "general", type = "qq")
  dev.off()
} else {
  warning("No results found in the data object. Please check the GWAS analysis.")
}